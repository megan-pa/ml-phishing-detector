...
Alongside the ML model, this system makes use of a rule-system to assist in classifying legitimate emails. A risk score is computed using factors surrounding links, language and formatting of emails, with each having a different weighted score. These scores are then combined and when exceeding a certain threshold (e.g. >= 5), the email is automatically classified as phishing regardless of the model's prediction. 
